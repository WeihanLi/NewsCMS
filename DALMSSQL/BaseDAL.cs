...
namespace DALMSSQL
{
    public class BaseDAL<T> : IDAL.IBaseDAL<T> where T : class, new()
    {
        ...
        public List<T> GetListBy<TKey>(Expression<Func<T, bool>> whereLambda, Expression<Func<T, TKey>> orderLambda, bool isAsc)
        {
            ...
            if (isAsc)
            {
                listData = db.Set<T>().Where(whereLambda).AsNoTracking().OrderBy(orderLambda).ToList();
            }
            else
            {
                listData = db.Set<T>().Where(whereLambda).AsNoTracking().OrderByDescending(orderLambda).ToList();
            }
            return listData;
        }
        #endregion
